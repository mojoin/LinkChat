...
static void reArmEpoll(socket_t fd, int epoll_fd)
{
    struct epoll_event cev;
    cev.events = EPOLLIN | EPOLLET | EPOLLRDHUP;
    if (send_buffers.find(fd) != send_buffers.end()) cev.events |= EPOLLOUT;
    cev.data.fd = fd;
...
static void flushSendBuf(socket_t fd, int epoll_fd)
{
    ...
    // Phase 1：后置清理 —— 上一次已发完 FIN 且发送缓冲已空
    auto it_st = file_state.find(fd);
    if (it_st != file_state.end() && it_st->second.eof_sent && send_buffers.find(fd) == send_buffers.end())
    {
        file_state.erase(it_st);
        cout << "fd=" << fd << " download fully done, state cleared" << endl;
    }

    // Phase 2：先排空"上次 trySend 没塞完的尾巴"
    ...
    if (it != send_buffers.end())
    {
        ...
        {
            cerr << "send() failed fd=" << fd << " errno=" << errno << endl;
        }

        if (send_buffers.find(fd) != send_buffers.end())
        {
            reArmEpoll(fd, epoll_fd); // 还有尾巴 → 等下次 EPOLLOUT
            return;                   // 绝不在有尾巴时再 trySend
        }
        // 尾巴清完了，才允许往下走
    }

    // Phase 3：泵新的（send_buffers 已空，trySend 调用顺序 = wire 顺序）
    if (clients[fd].mode == MODE_BINARY_DOWNLOAD)
    {
        auto &st = file_state[fd];
        constexpr int CHUNK = 32 * 1024;
        char sendbuf[CHUNK + 4];

        while (!st.eof_sent)
        {
            st.file_in.read(sendbuf + 4, CHUNK);
            int got = (int)st.file_in.gcount();

            uint32_t len_be = htonl((uint32_t)got);
            memcpy(sendbuf, &len_be, 4); // 避免 (uint32_t*)强转的对齐 UB
            ...
            if (sr == 1)
            {
                // 内核缓冲满：本帧尾巴进了 send_buffers，等 EPOLLOUT 再续
                break; // 落到 Phase 4 重设 EPOLLOUT
            }
            ...
            if (got == 0)
            {
                ...
                cout << "fd=" << fd << " download sent FIN, transfer="
                     << st.transfer_id << endl;
                break; // 落到 Phase 4
            }
            // sr==0 且 got>0：继续 while 泵下一帧（不变量 2 的体现）
        }
    }

    // Phase 4：唯一一处 epoll 重设
    reArmEpoll(fd, epoll_fd);
}
                          ...
                          map<socket_t, string> &recv_buffers)
{
    ...
    {
        if (it->second.file_out.is_open())
            it->second.file_out.close();
        if (it->second.file_in.is_open())
            it->second.file_in.close();
        // upload 半截断了: 删 .tmp
        if (!it->second.transfer_id.empty() && clients[fd].mode == MODE_BINARY_UPLOAD)
        {
            std::error_code ec;
            std::filesystem::remove(
                FileStore::convDirFor(it->second.from_uid, it->second.to_uid) + "/" + it->second.transfer_id + ".tmp", ec);
        }
        file_state.erase(it);
    }
                        ...
                        map<socket_t, string> &recv_buffers)
{
    ...
    if (info.state == HANDSHAKED)
    {
        ...
        try
        {
            ...
            if (j.value("type", "") == "login_reply" && j.value("ok", false) == true)
            {
                login_ok = true;
    ...
    // ===== 新增:下载模式主动 kick =====
    // JSON 回包已经 trySend 完毕(或缓冲进 send_buffers),
    // 此刻再推二进制,wire 顺序才是 JSON\n → 二进制帧
    if (clients.count(fd) && clients[fd].mode == MODE_BINARY_DOWNLOAD)
    {
        flushSendBuf(fd, epoll_fd);
        if (clients.find(fd) == clients.end())
            return false;   // flushSendBuf 内已 cleanup
    }
        ...
        for (int i = 0; i < nready; i++)
        {
            ...
            if (fd == sock)
            {
                ...
                while (true)
                {
                    ...
                    if (sr > 0)
                        cev.events |= EPOLLOUT; // 如果发送缓冲区满，注册可写事件
                 ...
                 const std::string &saved_name)
{
    ...
    std::string tmp_path = FileStore::convDirFor(from_uid, to_uid) + "/" + transfer_id + ".tmp";
...
static void finalizeUpload(socket_t fd)
{
    ...
    // .tmp → final
    std::string tmp_path = FileStore::convDirFor(st.from_uid, st.to_uid) + "/" + st.transfer_id + ".tmp";
    std::string final_path = FileStore::savedPathFor({st.transfer_id, st.from_uid, st.to_uid,
...
{
    auto rec = FileStore::findByTransferId(from_uid, peer_uid, transfer_id);
    if (!rec)
    {
