...
namespace
{
    ...
    string handleDeleteFile(const json &req, int from_uid)
    {
        ...
        if (!req.contains("peer_uid") || !req["peer_uid"].is_number_integer())
            return makeError("delete_file: missing or invalid 'peer_uid' field");
...
string handleMessage(const std::string &line, int from_uid, int fd)
{
    ...
    if (type == "get_friends")
        return handleGetFriends(req);
    if (type == "register")
        return handleRegister(req);
    if (type == "chat")
        return handleChat(req, from_uid);
    if (type == "get_history")
        return handleGetHistory(req);
    if (type == "list_files")
        return handleListFiles(req, from_uid);
    if (type == "upload_file")
        return handleUploadFile(req, from_uid, fd);
